...
| `NUM_ESTIMATORS`| 20 | Số lượng weak learners (experts) trong ensemble. |
| `BATCH_SIZE` | 16 | Kích thước batch cho huấn luyện. |
| `NUM_EPOCHS` | 10 | Số epoch huấn luyện cho mỗi weak learner. |
| `CLIENT_LR` | 1e-4 | Tốc độ học cho các weak learners. |
| `WEIGHT_DECAY` | 1e-2 | Hệ số regularization cho các weak learners. |

### 1.2 Thông số Gating Network
| Biến | Giá trị | Ý nghĩa |
| :--- | :--- | :--- |
| `GATING_K` | 14 | Số lượng experts được chọn trong Sparse Inference. |
...
### 1.3 Quản lý Dữ liệu miền Đích
Dữ liệu miền đích hiện được quản lý thông qua các file riêng biệt:
- **Training Set**: Tập dữ liệu dùng để huấn luyện (Labeled).
- **Test Set**: Tập dữ liệu độc lập dùng để đánh giá cuối cùng.
...
### 2.2 Chiến lược Tải dữ liệu
Thay vì chia từ một file duy nhất, hệ thống tải dữ liệu trực tiếp từ các file feather:
1. **`load_target_train_data(path)`**: Tải tập huấn luyện miền đích.
2. **`load_target_test_data(path)`**: Tải tập kiểm tra miền đích.
...
- **Attention Pooling**: Gating Network sử dụng lớp Flatten để chuyển đổi feature map cuối cùng thành vector phẳng trước khi đưa vào lớp Fully Connected. Điều này đảm bảo toàn bộ thông tin không gian từ các lớp Convolution được giữ lại để đưa ra quyết định routing chính xác nhất.
- **Final Layer**: `Linear(flatten_dim, num_learners)` $\rightarrow$ Output điểm số cho từng expert.

---

## 4. Chi tiết Training Pipeline
Quy trình huấn luyện được chia thành các giai đoạn: thiết lập baseline, xây dựng đội ngũ chuyên gia (Ensemble) và huấn luyện người điều phối (Gating Network).

### Giai đoạn 0: Huấn luyện Baseline CNN Model
*Mục tiêu: Tạo ra một model tiêu chuẩn để làm mốc so sánh hiệu năng.*
- **Huấn luyện**: Một model CNN được huấn luyện trực tiếp trên tập hợp dữ liệu kết hợp giữa nguồn và đích.
- **Đánh giá**: Kết quả của model này được dùng làm baseline về độ chính xác và thời gian suy luận.
...
2. **Vòng lặp huấn luyện tuần tự (t = 1 $\rightarrow$ T)**:
   ...
   - **Huấn luyện**: Train một model CNN trên tập mẫu này với `weight_decay` để tránh overfitting $\rightarrow$ Thu được Chuyên gia $t$.
...
- **Tạo nhãn Oracle**: 
  - **Sử dụng Xác suất (Soft Labels)**: Thay vì chỉ dùng kết quả đúng/sai nhị phân, hệ thống tính toán đóng góp của mỗi chuyên gia dựa trên xác suất softmax của lớp đúng. 
  - **Công thức**: $\text{Contribution}_{i,t} = P(\text{class}_{true} | \text{Expert}_t, \mathbf{x}_i)$. Điều này giúp Gating Network học được mức độ tự tin của mỗi expert đối với từng mẫu dữ liệu cụ thể.
- **Huấn luyện**:
  - **Task Loss**: BCE loss giữa output của Gating Net và nhãn Oracle.
...
1. **Load Data**: Gọi `load_source_data`, `load_target_train_data` và `load_target_test_data`.
2. **Phase 0 (Baseline)**: Huấn luyện và đánh giá Baseline CNN Model $\rightarrow$ Lưu `model_baseline.pth`.
3. **Phase 1 (Original)**: Chạy `model_orig.fit` $\rightarrow$ Lưu `model_orig.pth`.
4. **Phase 2 (Gating)**:
    - Khởi tạo `model_gated` với kiến trúc `GatingCNN`.
    - Huấn luyện Gating Network sử dụng nhãn Oracle dựa trên xác suất.
    - Lưu `model_gated.pth`.
5. **Phase 3 (Evaluation)**: 
    - So sánh Accuracy và Thời gian suy luận giữa: Baseline CNN vs Full Ensemble vs Gated Sparse (với `GATING_K`).
